...
import { Trash2, Edit, Plus, Search, User, Users, Loader2 } from 'lucide-react';
...
import { useDependentes, useDependenteMutations, useClientes } from '@/hooks/useQueries';
...
export const DependentesTab: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.nome || !formData.cpf || !formData.clienteId) {
      toast.error('Por favor, preencha todos os campos obrigatórios.');
      return;
    }

    // setIsLoading(true);
    const toastId = toast.loading('Salvando dependente...');

    try {
      if (editingItem) {
        await updateDependente.mutateAsync({ id: editingItem.id, data: formData });
        toast.success('Dependente atualizado com sucesso!', { id: toastId });
      } else {
        await createDependente.mutateAsync(formData);
        toast.success('Dependente criado com sucesso!', { id: toastId });
      }
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Tem certeza que deseja excluir este dependente?')) {
      try {
        await removeDependente.mutateAsync(id);
      } catch (error) {
