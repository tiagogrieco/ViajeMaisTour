...
import { Trash2, Edit, Plus, Search, Package, DollarSign, Loader2 } from 'lucide-react';
import { Produto } from '@/types';
import { formatDate, formatCurrency } from '@/utils/formatters';
import { supabaseService } from '@/services/supabaseService';
import { toast } from 'sonner';

import { useProdutos, useProdutoMutations } from '@/hooks/useQueries';

export const ProdutosTab: React.FC = () => {
  const { data: produtos = [], isLoading: isLoadingProdutos } = useProdutos();
  const { create: createProduto, update: updateProduto, remove: removeProduto } = useProdutoMutations();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        await updateProduto.mutateAsync({ id: editingItem.id, data: formData });
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Tem certeza que deseja excluir este produto?')) {
      try {
        await removeProduto.mutateAsync(id);
      } catch (error) {
