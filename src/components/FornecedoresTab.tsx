...
import { Trash2, Edit, Plus, Search, Building, Phone, Mail, MapPin, Loader2 } from 'lucide-react';
...
import { useFornecedores, useFornecedorMutations } from '@/hooks/useQueries';

export const FornecedoresTab: React.FC = () => {
  const { data: fornecedores = [], isLoading: isLoadingFornecedores } = useFornecedores();
  const { create: createFornecedor, update: updateFornecedor, remove: removeFornecedor } = useFornecedorMutations();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        await updateFornecedor.mutateAsync({ id: editingItem.id, data: formData });
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Tem certeza que deseja excluir este fornecedor?')) {
      try {
        await removeFornecedor.mutateAsync(id);
      } catch (error) {
